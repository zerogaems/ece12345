...
# ==================== الإعدادات الأساسية ====================
# جلب التوكن ومعرف الأدمن من متغيرات البيئة (Environment Variables) في Render
...
# ملاحظة: أضف البوت كمشرف (Admin) في هذه القنوات/المجموعات مع صلاحية "دعوة المستخدمين عبر رابط"
YEAR_CHATS = {
...
def handle_excel_upload(message):
    ...
    msg = bot.reply_to(message, "⏳ جاري معالجة الملف، تنظيف البيانات، وتحديث قاعدة البيانات...")
    ...
    try:
        ...
        bot.edit_message_text(f"❌ حدث خطأ أثناء معالجة الملف: {str(e)}", chat_id=message.chat.id, message_id=msg.message_id)
...
def start_command(message):
    ...
    # 🔒 حماية رقم 1: الفحص المباشر بـ Telegram User ID لعدم تكرار الطلب من نفس الحساب
    conn = sqlite3.connect("telecom_students.db")
    cursor = conn.cursor()
    cursor.execute("SELECT name, year, joined FROM students WHERE telegram_user_id = ?", (user_id,))
    ...
    button = types.KeyboardButton(text="📱 مشاركة رقم التليغرام لتأكيد الهوية", request_contact=True)
    markup.add(button)

    welcome_text = (
        "أهلاً بك في البوت الرسمي لقسم الهندسة الإلكترونية والاتصالات (الهمك) 🎓\n\n"
        "للحصول على رابط المجموعة/القناة الخاصة بسنتك الدراسية، يرجى الضغط على الزر أدناه لمشاركة رقمك المعتمد في الاستبيان."
    )
...
def handle_contact(message):
    ...
    # 🔒 حماية رقم 2: الفحص بـ Telegram User ID للتأكد من عدم استخدام الحساب سابقاً
    cursor.execute("SELECT name, year, joined FROM students WHERE telegram_user_id = ?", (user_id,))
    already_joined_account = cursor.fetchone()
    if already_joined_account and already_joined_account[2] == 1:
        bot.send_message(
            message.chat.id,
            f"🚫 **عذراً! حسابك التليغرام مسجّل ومستلم لرابط الانضمام سابقاً.**",
    ...
    cursor.execute("SELECT student_id, name, year, joined FROM students WHERE phone = ?", (cleaned_phone,))
    ...
    if not student:
        bot.send_message(
            ...
            "يرجى التأكد من تعبئة الاستبيان بنفس هذا الرقم، أو التواصل مع الهيئة لمراجعة بياناتك.",
            reply_markup=types.ReplyKeyboardRemove(),
            parse_mode="Markdown"
        )
        conn.close()
    ...
    if joined == 1:
        bot.send_message(
            message.chat.id,
            f"⚠️ **عذراً، هذا الرقم الجامعي ({student_id}) استلم رابط الانضمام سابقاً ولا يمكن استخدامه مجدداً.**",
            reply_markup=types.ReplyKeyboardRemove(),
            parse_mode="Markdown"
        )
        conn.close()
    ...
    if not chat_target_id:
        bot.send_message(message.chat.id, "⚠️ خطأ في إعدادات السنة الدراسية، يرجى مراجعة الأدمن.")
        conn.close()
        return
    ...
    try:
        ...
        success_msg = (
            ...
            f"🔗 **رابط الانضمام الخاطف (صالح لاستخدام شخص واحد ولمرة واحدة فقط):**\n"
            f"{invite_link.invite_link}"
        )
        bot.send_message(message.chat.id, success_msg, reply_markup=types.ReplyKeyboardRemove(), parse_mode="Markdown")

    except Exception as e:
        bot.send_message(message.chat.id, f"❌ حدث خطأ أثناء إنشاء رابط الانضمام: {str(e)}")
...
def admin_stats(message):
    ...
    cursor.execute("SELECT year, COUNT(*), SUM(joined) FROM students GROUP BY year")
    year_stats = cursor.fetchall()
    conn.close()

    text = f"📊 **إحصائيات الانتقال للتلغرام:**\n\n"
